...
def largest_product_of_n_digits_in_list_of_numbers(list_of_numbers: list, number_of_digits: int) -> int:
    """ Search for biggest product of digits in big number given by a list."""
    n = number_of_digits
    maximum = 1
    temp = 1
    while True:
...
def Largest_product_in_a_grid(list_of_numbers: list, number_of_factors: int) -> int:
    """ Search for the greatest productin a grid.
    
        Search for the greatest product of four adjacent numbers 
...
def up_down_product(list_of_numbers: list, number_of_factors: int) -> int:
    """ Search for the greatest up-down productin a grid.
        
        Search for the greatest product of four adjacent numbers
        in the up-down direction in the 20×20 grid."""
    maximum = 0
...
def left_right_product(list_of_numbers: list, number_of_factors: int) -> int:
    """ Search for the greatest left-right productin a grid.
    
        Search for the greatest product of four adjacent numbers
        in the left-right direction in the 20×20 grid."""
    maximum = 0
    for i in range(len(list_of_numbers)):
...
def diagonally_r_l_product(list_of_numbers: list, number_of_factors: int) -> int:#FIXME
    """ Search for the greatest diagonally right-to-left product in a grid.
        
        Search for the greatest product of four adjacent numbers
        in the diagonally right-left direction in the 20×20 grid."""
    maximum = 0
    """#FIXME work only for square grid
        min_list_len = len(list_of_numbers)
        for i in range(len(list_of_numbers)):
            if len(list_of_numbers[i]) < min_list_len:
                min_list_len = len(list_of_numbers)
        if len(list_of_numbers) < number_of_factors:
            return maximum
    """
    ...
    """
        for n in range(number_of_factors-1, len(list_of_numbers)):
            ...
            temp_list_2 = []
            for i in range(0, n, 1):
                for j in range(n, -1, -1):#FIXME I cannot use j
                    temp_list_1.append(list_of_numbers[i][j])
                    temp_list_2.append(list_of_numbers[j][i])#FIXME bad indexes
            temp = largest_product_of_n_digits_in_list_of_numbers(temp_list_1, number_of_factors)
            if maximum < temp: maximum = temp
            temp = largest_product_of_n_digits_in_list_of_numbers(temp_list_2, number_of_factors)
            if maximum < temp: maximum = temp
...
def diagonally_l_r_product(list_of_numbers: list, number_of_factors: int) -> int:#FIXME
    """ Search for the greatest diagonally left-to-right product in a grid.
        
        Search for the greatest product of four adjacent numbers
        in the diagonally left-right direction in the 20×20 grid."""
    maximum = 0
    """#FIXME work only for square grid
        min_list_len = len(list_of_numbers)
        for i in range(len(list_of_numbers)):
...
if __name__ == "__main__":
    ...
    n_element = ''
    for i in range(len(lines)):#lines in a grid
